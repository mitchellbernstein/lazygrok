---
name: hyperplan
description: "[LazyGrok] Adversarial multi-agent planning skill. Self-orchestrates 5 hostile category members (unspecified-low, unspecified-high, deep, ultrabrain, artistry) via parallel Grok subagents for ruthless cross-critique debate, distills only the defensible insights, then MANDATORILY hands the distilled insight bundle to the `plan` agent for executable plan formalization. Use when planning needs maximum rigor and surfacing of weak assumptions, blind spots, and over-engineering. Triggers: 'hyperplan', 'hpp', '/hyperplan', 'adversarial plan', 'hostile planning', 'cross-critique plan'. LazyGrok commands: /lg-hyperplan, /hpp."
---
...
1. **`spawn_subagent` is available** (Grok Build). Hyperplan on LazyGrok uses parallel `spawn_subagent` waves; the parent brokers all cross-critique. No OpenCode `team_*` tools required.
2. **You are the main session lead** (not a nested planner-only child). If you were spawned as `lg-plan` only, tell the user to run `/hyperplan` from the main chat, or hand off after distillation.
3. **You will not implement product code.** Hyperplan distills + formalizes a plan only.
...
Spawn **5 parallel** `spawn_subagent` children (`background: true`). Use `subagent_type: "general-purpose"` (or `lg-worker` with read-only prompts) and put the **full adversarial system prompt** for that lens at the top of `prompt`, then the Round-1 task.

| Member name | Lens |
| --- | --- |
| skeptic | Pragmatist Skeptic (full system prompt above) |
| validator | Integration Tester |
| researcher | Autonomous Researcher |
| architect | Architect Strategist |
| creative | Creative Challenger |

Track each `subagent_id`. If a spawn fails, continue with remaining members and note degraded roster (never drop skeptic/architect if avoidable).

Record roster in todos. Do **not** use OpenCode `team_create`.

### Phase 2: Round 1 — Independent analysis

Send the same Round-1 task to all 5 members via 5 parallel `spawn_subagent` calls (include their lens system prompt + task). Each member receives:
...
When done, return findings as the spawn deliverable to the parent.
</hyperplan-round-1-task>
```

**[WAIT]** Poll with `get_command_or_subagent_output` until all five complete (or mark missing).
...
Re-spawn (or resume) all 5 members via parallel `spawn_subagent` with the SAME findings bundle. Each prompt is:
...
When done, return attacks as the spawn deliverable.
</hyperplan-round-2-task>
```

**[WAIT]** Poll until all cross-attacks return.
...
When done, return defense/refine/concede results to parent.
</hyperplan-round-3-task>
```

**[WAIT]** Poll until all refinements return.
...
(If the host only supports `background: true`, spawn and wait with `get_command_or_subagent_output`.)
...
After the plan agent output has been presented:

1. Optionally `kill_command_or_subagent` any still-running children.
2. Confirm to the user: "Hyperplan squad complete (no team runtime to delete on Grok)."
...
| Using OpenCode `team_*` or Codex `multi_agent_v1.*` | Use Grok `spawn_subagent` only. |
| Forgetting the `lg-plan` handoff | Distillation is not the plan — always spawn `lg-plan` with the insight bundle. |
| Running nested inside a plan-only child | Prefer main session lead orchestration. |
| Skipping adversarial rounds to save tokens | That is vanilla planning — not hyperplan. |

## NOTES FOR THE LEAD (YOU)

- Each `spawn_subagent` is fire-and-forget until you poll. Children do not see each other.
- After spawning Round-N, poll with `get_command_or_subagent_output` until complete.
- Track `subagent_id`s and poll each; re-spawn stragglers once if needed.
- Members never see each other. You are the information broker. Bundles you paste into Phase 3/4 prompts are their entire context.
