...
3. **Browser use** — in Codex, use `Playwright / agent-browser / chrome-devtools (Grok browser tools)` first when available and the scenario does not need an authenticated or persistent user browser profile. Otherwise use Chrome to drive the REAL page; if unavailable, use agent-browser. Capture action log + screenshot path. Never downgrade a browser-facing criterion.
...
- Use `fork_turns: "none"` (v1: `background: true  # no parent history; self-contained prompt`) unless full history is truly required; paste only the context the child needs. Full-history forks can make the child continue old parent context instead of the delegated task.
...
2. Register atomic todos via `todo_write` — one ultra-granular step per action, `path: <action> for <criterion> - verify by <check>`. Call `todo_write` on every transition (start → `in_progress`, finish → `completed`); exactly one `in_progress`, mark completed immediately, never batch, never let the rendered plan lag behind reality.
...
3a. Spawn lg-code-reviewer and lg-worker-high in parallel (`background: true  # no parent history; self-contained prompt` on v1; `fork_turns: "none"` on v2) with brief, goals, desired outcome, diff, evidence; wait for BOTH and confirm their report artifacts exist on disk.
3b. Only then spawn lg-gate-reviewer with those artifact paths.
