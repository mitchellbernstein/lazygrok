...
Dispatch through your harness's own subagent tool. In OpenCode: `task(subagent_type="lg-gate-reviewer", ...)`. In Codex: `spawn_subagent({"message": "...", "agent_type": "lg-gate-reviewer", "background": true})` (the code blocks below are written in OpenCode `task(...)` form; translate them to that `spawn_agent` call, putting the full prompt in `message`).
...
```
task(subagent_type="lg-gate-reviewer",
...
2. Code-level design-system fidelity (code oracle). Dispatch through your harness's own subagent tool.
   ...
   **Codex:** `spawn_subagent({"message":"TASK: Act as a clone / design-system fidelity reviewer. ...","agent_type":"lazycodex-clone-fidelity-reviewer","background": true})`
